...
// 切回 Claude 時呼叫 codex / gemma 自我總結，產出帶回主對話的 context
const CODEX_SUMMARIZE_PROMPT =
  '請用繁體中文總結這段對話（100-250 字，可條列可段落），聚焦於\n' +
  '(1) 已得出的結論或決定\n' +
  '(2) 尚未解決的問題或下一步待辦\n' +
  '(3) 關鍵程式碼/檔名/設定值（原樣保留，不要簡化）\n' +
  '這份摘要會被注入到我與另一個 AI 的主對話作為脈絡，' +
  '請以第三人稱描述事實，不要對使用者說話、不要寒暄、' +
  '不要用 markdown 標題、不要使用 emoji。';
const GEMMA_SUMMARIZE_PROMPT =
  '用繁體中文總結剛才的對話（100-250 字）：結論、待辦、關鍵字。' +
  '這段會給另一個 AI 看，請以第三人稱描述事實。' +
  '不要寒暄、不要 markdown、不要 emoji。';
const SUMMARIZE_TIMEOUT_MS = 30000; // codex 路徑兜底，gemma 由 gemmaExecute 自帶逾時
const SUMMARIZE_MIN_TURNS  = 2;     // <= 2 輪維持原本「最後一句」行為
...
async function runClaude(prompt, chatId, forceNew = false, imagePaths = []) {
  ...
  const gemmaSection = gemmaCtx
    ? `[以下是使用者剛才在 Gemma（本機小模型）端得到的脈絡資訊（可能是摘要或最後一段輸出），僅供參考脈絡，請根據使用者接下來的訊息回應]\n${gemmaCtx}\n\n---\n\n`
    : '';
  // 若剛從 codex 切回，前置 codex 最後一句輸出（一次性，用完清空）
  const codexCtx = pendingCodexContext;
  pendingCodexContext = null;
  const codexSection = codexCtx
    ? `[以下是使用者剛才在 Codex（OpenAI codex-cli）端得到的脈絡資訊（可能是摘要或最後一段輸出），僅供參考脈絡，請根據使用者接下來的訊息回應]\n${codexCtx}\n\n---\n\n`
    : '';
  ...
  if (data.startsWith('model:')) {
    ...
    if (choice === 'gemma') {
      ...
      if (gemmaMode) {
        ...
        // 切換完丟棄 codex 歷史；codex→gemma 不做反向 bridge
        codexHistory = [];
      } else {
        // 關閉時：> SUMMARIZE_MIN_TURNS 自動 summarize，否則 bridge 最後一句
        const { bridge, note } = await bridgeFromGemma(chatId, msgId);
        if (bridge) {
          pendingGemmaContext = bridge;
          inheritNote = note;
        }
        gemmaHistory = [];
      }
      // 若是從 codex 切 gemma，丟掉 codex 歷史
      if (wasCodex) codexHistory = [];
      ...
      // 從 gemma / codex 切回 Claude model 時：> SUMMARIZE_MIN_TURNS 自動 summarize，否則 bridge 最後一句
      // bridge 必須在 clear history 之前呼叫，否則 history 已空
      let ctxNote = '';
      if (gemmaMode) {
        const { bridge, note } = await bridgeFromGemma(chatId, msgId);
        if (bridge) { pendingGemmaContext = bridge; ctxNote += note; }
      }
      if (codexMode) {
        const { bridge, note } = await bridgeFromCodex(chatId, msgId);
        if (bridge) { pendingCodexContext = bridge; ctxNote += note; }
      }
      gemmaMode = false;
      gemmaHistory = [];
      codexMode = false;
...
// 設計：> SUMMARIZE_MIN_TURNS 才呼叫 backend 自我總結；否則 fallback 到「最後一句」。
// 任何錯誤（exit ≠ 0、timeout、空輸出）都 fallback，不阻擋模型切換。
async function summarizeCodexConversation() {
  if (!runCodex) return null;
  const cwdMatches = currentCodexThreadCwd && currentCodexThreadCwd === workingDir;
  const useResume  = !!currentCodexThreadId && cwdMatches;

  let promptForCodex;
  if (useResume) {
    promptForCodex = CODEX_SUMMARIZE_PROMPT;
  } else {
    const recent = codexHistory.slice(-CODEX_HISTORY_TURNS * 2);
    const block = recent.map(m => `${m.role === 'user' ? 'User' : 'Codex'}: ${m.content}`).join('\n\n');
    promptForCodex = '[以下是剛才的對話歷史]\n' + block + '\n\n---\n\n' + CODEX_SUMMARIZE_PROMPT;
  }
...
async function summarizeGemmaConversation() {
  if (!gemmaExecute) return null;
  try {
    const result = await gemmaExecute({
      prompt: GEMMA_SUMMARIZE_PROMPT,
...
async function bridgeFromCodex(chatId, msgId) {
  ...
  let summary = null;
  if (turns > SUMMARIZE_MIN_TURNS) {
    await bot.editMessageText('⏳ 正在請 Codex 總結這段對話…', { chat_id: chatId, message_id: msgId }).catch(() => {});
  ...
  if (lastAssistant?.content) {
    return { bridge: lastAssistant.content, note: '\n\n📎 下一則訊息將自動附帶 Codex 最後一句輸出給 Claude' };
  }
  return { bridge: null, note: '' };
}

async function bridgeFromGemma(chatId, msgId) {
  const turns = gemmaHistory.length / 2 | 0;
  let summary = null;
  if (turns > SUMMARIZE_MIN_TURNS) {
    await bot.editMessageText('⏳ 正在請 Gemma 總結這段對話…', { chat_id: chatId, message_id: msgId }).catch(() => {});
...
function buildCodexPrompt(userPrompt, { useResume = false, isOneShot = false } = {}) {
  ...
  if (codexHistory.length) {
    const recent = codexHistory.slice(-CODEX_HISTORY_TURNS * 2);
    const histText = recent.map(m => {
      const tag = m.role === 'user' ? 'User' : 'You (Codex)';
      return `${tag}: ${m.content}`;
    }).join('\n\n');
    parts.push('[以下是你跟使用者剛才的對話歷史]\n' + histText);
  }
  if (!parts.length) return userPrompt;
  return parts.join('\n\n---\n\n') + '\n\n---\n\n[使用者的新訊息]\n' + userPrompt;
}
...
bot.on('message', async (msg) => {
  ...
  if (msg.text && !msg.text.startsWith('/')) {
    ...
    if (dirBtn) {
      ...
      newSession = true;
      currentSessionId = null;
      const _label = cwdToLabel(workingDir);
      const _sessions = loadAllSessions(_label);
      if (_sessions.length > 0) {
        bot.sendMessage(msg.chat.id,
          `✅ 已切換至：\`${workingDir}\``,
          { parse_mode: 'Markdown', reply_markup: MAIN_KEYBOARD });
        bot.sendMessage(msg.chat.id,
          `📑 選擇 Session 或直接輸入開始新對話（共 ${_sessions.length} 則）：`,
          { reply_markup: buildSessionsKeyboard(_sessions, 0) });
      } else {
        bot.sendMessage(msg.chat.id,
          `✅ 已切換至：\`${workingDir}\`\n🆕 將使用新 Session`,
          { parse_mode: 'Markdown', reply_markup: MAIN_KEYBOARD });
