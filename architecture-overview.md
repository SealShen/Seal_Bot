...
## 四、Prompt Router（prompt_router.py）

UserPromptSubmit hook `prompt_router.py` 取代原 AgentOpt 風格 model router（`route_model.py` / `task_router.py` 已於 2026-04-22 廢棄）。現以 LLM 分類器（Google AI Studio Gemma/Flash cascade → 本地 LM Studio fallback）為每則 prompt 注入：

- `[PROMPT_ROUTER] subagent=<type> source=<s>` → 主 Claude 改呼叫 `Agent(subagent_type=...)`
- `[PROMPT_ROUTER] delegate=gemma_chat source=<s>` → 委派到 `mcp__gemma-local__gemma_chat`

分類規則與採納行為詳見 [CLAUDE.md](CLAUDE.md) 的「Prompt Router subagent 指派」章節。決策 log 寫入 `~/.claude/routing-decisions.log`，稽核工具 `routing_report.py`。
